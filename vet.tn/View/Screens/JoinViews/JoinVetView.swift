...
struct JoinVetView: View {
    ...
    private var canSubmit: Bool {
        let baseValid = isNameValid && isLicenseValid && isClinicValid &&
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 18) {
                    ...
                    // Passwords (only for new registrations)
                    if session.user == nil {
                    ...
                    Button {
                        ...
                        if isSubmitting {
                            ...
                            Text(session.user != nil ? "CONVERT TO VETERINARIAN" : "REGISTER CLINIC")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 18)
                            .fill(canSubmit && !isSubmitting ? Color.vetCanyon : Color.vetCanyon.opacity(0.4))
                    )
    ...
    @MainActor
    private func submitVetForm() async {
        isSubmitting = true
        errorMessage = nil
        ...
        let trimmedAddress = clinicAddress.trimmingCharacters(in: .whitespacesAndNewlines)
        let trimmedBio = bio.isEmpty ? nil : bio.trimmingCharacters(in: .whitespacesAndNewlines)
        
        let specsArray = selectedSpecs.map { $0.rawValue }
        ...
        do {
            ...
            if let currentUser = session.user {
                ...
                // Convert existing user to vet
                do {
                    let updatedUser = try await service.convertUserToVet(
                        userId: currentUser.id,
                        licenseNumber: licenseNumber,
                        clinicName: clinicName,
                        clinicAddress: trimmedAddress,
                        specializations: specsArray.isEmpty ? nil : specsArray,
                    ...
                    // Always require email verification when converting to vet
                ...
                let newUser = try await service.createVet(request)
            ...
            #if DEBUG
            print("❌ Failed to register vet: \(error)")
            #endif
        }
