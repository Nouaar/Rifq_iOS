...
struct SplashView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Fallback: if backend/render is slow, force proceed shortly after
            DispatchQueue.main.asyncAfter(deadline: .now() + totalDuration + 1.2) {
                maybePresent(forceIfAnimationDone: true)
            }
        }
        .onChange(of: session.hasRestoredSession) { _ in
            maybePresent()
        }
        .onChange(of: session.isAuthenticated) { _ in
            maybePresent()
        }
        .fullScreenCover(isPresented: $goNext) {
            // Decide destination based on authentication & verification state
            if session.requiresEmailVerification {
                EmailVerificationView(
                    email: session.pendingEmail ?? session.user?.email ?? ""
                )
    ...
    private func maybePresent(forceIfAnimationDone: Bool = false) {
        guard !presented else { return }
        if animationDone && (session.hasRestoredSession || forceIfAnimationDone) {
            presented = true
            goNext = true
        }
    }
