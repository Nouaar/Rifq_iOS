...
final class GoogleAuthViewModel: ObservableObject {
    ...
    func signIn(session: SessionManager, source: Source = .login) {
        ...
        GIDSignIn.sharedInstance.signIn(withPresenting: rootVC) { result, error in
            ...
            // Read email from Google result if available (used for backend checks)
            let googleEmail = (user.profile?.email ?? "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            ...
            Task { @MainActor in
                switch source {
                case .signup:
                    await session.googleSignup(idToken: idToken, email: googleEmail)
                case .login:
                    await session.googleSignin(idToken: idToken, email: googleEmail)
                }
